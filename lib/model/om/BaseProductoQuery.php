 ...
 * @method ProductoQuery orderByCantidadMinimaPedido($order = Criteria::ASC) Order by the cantidad_minima_pedido column
 *
 ...
 * @method ProductoQuery groupByCantidadMinimaPedido() Group by the cantidad_minima_pedido column
 *
 ...
 *
 * @method ProductoQuery leftJoinMovimiento($relationAlias = null) Adds a LEFT JOIN clause to the query using the Movimiento relation
 * @method ProductoQuery rightJoinMovimiento($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Movimiento relation
 * @method ProductoQuery innerJoinMovimiento($relationAlias = null) Adds a INNER JOIN clause to the query using the Movimiento relation
 *
 ...
 * @method Producto findOneByCantidadMinimaPedido(int $cantidad_minima_pedido) Return the first Producto filtered by the cantidad_minima_pedido column
 *
 ...
 * @method array findByCantidadMinimaPedido(int $cantidad_minima_pedido) Return Producto objects filtered by the cantidad_minima_pedido column
 *
...
abstract class BaseProductoQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `DESCRIPCION`, `MARCA_ID`, `TIPO_PRODUCTO_ID`, `TIPO_PRESENTACION_ID`, `CANTIDAD_MINIMA`, `IMAGEN`, `CODIGO`, `PRECIO`, `COSTO`, `CANTIDAD_MINIMA_PEDIDO` FROM `producto` WHERE `ID` = :p0';
        try {
    ...
    /**
     * Filter the query on the cantidad_minima_pedido column
     *
     * Example usage:
     * <code>
     * $query->filterByCantidadMinimaPedido(1234); // WHERE cantidad_minima_pedido = 1234
     * $query->filterByCantidadMinimaPedido(array(12, 34)); // WHERE cantidad_minima_pedido IN (12, 34)
     * $query->filterByCantidadMinimaPedido(array('min' => 12)); // WHERE cantidad_minima_pedido > 12
     * </code>
     *
     * @param     mixed $cantidadMinimaPedido The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return ProductoQuery The current query, for fluid interface
     */
    public function filterByCantidadMinimaPedido($cantidadMinimaPedido = null, $comparison = null)
    {
        if (is_array($cantidadMinimaPedido)) {
            $useMinMax = false;
            if (isset($cantidadMinimaPedido['min'])) {
                $this->addUsingAlias(ProductoPeer::CANTIDAD_MINIMA_PEDIDO, $cantidadMinimaPedido['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($cantidadMinimaPedido['max'])) {
                $this->addUsingAlias(ProductoPeer::CANTIDAD_MINIMA_PEDIDO, $cantidadMinimaPedido['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(ProductoPeer::CANTIDAD_MINIMA_PEDIDO, $cantidadMinimaPedido, $comparison);
    }
    ...
    public function filterByMovimiento($movimiento, $comparison = null)
    {
        if ($movimiento instanceof Movimiento) {
            return $this
                ->addUsingAlias(ProductoPeer::ID, $movimiento->getProductoId(), $comparison);
