...
<th class="   sf_admin_<?php echo strtolower($field->getType()) ?> sf_admin_list_th_<?php echo $name ?>">
  ...
  [?php else: ?]
       [?php echo link_to(__('<?php echo '<span class="primary-info"> '.$field->getConfig('label', '', true).'</span>' ?>', array(), '<?php echo $this->getI18nCatalogue() ?>'), '@<?php echo $this->getUrlForAction('list') ?>', array('query_string' => 'sort=<?php echo $name ?>&sort_type=asc')) ?]    
  [?php endif; ?]
<?php else: ?>
  <span class="primary-info">   [?php echo __('<?php echo $field->getConfig('label', '', true) ?>', array(), '<?php echo $this->getI18nCatalogue() ?>') ?]</span>
<?php endif; ?>
